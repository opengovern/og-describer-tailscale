...
func GetDevicePosture(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		ID:   integration.ID,
		Name: integration.ID,
		Description: model.PostureIntegrationDescription{
			ID:            integration.ID,
...
func processDevicePostures(ctx context.Context, handler *resilientbridge.ResilientBridge, tailScaleChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var devicePostures model.ListPostureIntegrations
	...
	for _, integration := range devicePostures.Integrations {
		wg.Add(1)
		go func(integration model.PostureIntegrationJSON) {
			defer wg.Done()
			status := model.Status{
				LastSync:             integration.Status.LastSync,
				Error:                integration.Status.Error,
...
func processDevicePosture(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*model.PostureIntegrationJSON, error) {
	var devicePosture model.PostureIntegrationJSON
