...
func GetWebhook(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: provider.WebhookDescription{
...
func processWebhooks(ctx context.Context, handler *resilientbridge.ResilientBridge, tailScaleChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var webhookListResponse provider.ListWebhookResponse
	baseURL := "/v2/tailnet/-/webhooks"
	...
	for _, webhook := range webhookListResponse.Webhooks {
		wg.Add(1)
		go func(webhook provider.WebhookJSON) {
			defer wg.Done()
			value := models.Resource{
				ID:   webhook.EndpointID,
				Name: webhook.EndpointURL,
...
func processWebhook(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*provider.WebhookJSON, error) {
	var webhook provider.WebhookJSON
	baseURL := "/v2/webhooks/"
