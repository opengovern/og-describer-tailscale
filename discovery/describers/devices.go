...
func GetDevice(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*models.Resource, error) {
	...
	latencyMap := make(map[string]provider.Latency)
	...
	clientSupports := provider.ClientCapabilities{
	...
	var clientConnectivity provider.ClientConnectivity
	if device.ClientConnectivity != nil {
		clientConnectivity = provider.ClientConnectivity{
	...
	var postureIdentity provider.PostureIdentity
	if device.PostureIdentity != nil {
		postureIdentity = provider.PostureIdentity{
	...
	value := models.Resource{
		ID:   device.ID,
		Name: device.Name,
		Description: provider.DeviceDescription{
...
func processDevices(ctx context.Context, handler *resilientbridge.ResilientBridge, tailScaleChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var deviceListResponse provider.DeviceListResponse
	...
	for _, device := range deviceListResponse.Devices {
		wg.Add(1)
		go func(device provider.DeviceJSON) {
			defer wg.Done()
			latencyMap := make(map[string]provider.Latency)
			for key, value := range device.ClientConnectivity.Latency {
				latency := provider.Latency{
...
func processDevice(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*provider.DeviceJSON, error) {
	var device provider.DeviceJSON
	baseURL := "/v2/device/"
