...
func GetUserInvite(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: provider.UserInviteDescription{
			ID:              userInvite.ID,
...
func processUserInvites(ctx context.Context, handler *resilientbridge.ResilientBridge, tailScaleChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var userInvites []provider.UserInviteJSON
	...
	for _, userInvite := range userInvites {
		wg.Add(1)
		go func(userInvite provider.UserInviteJSON) {
			defer wg.Done()
			value := models.Resource{
				ID:   userInvite.ID,
				Name: userInvite.InviteURL,
...
func processUserInvite(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*provider.UserInviteJSON, error) {
	var userInvite provider.UserInviteJSON
